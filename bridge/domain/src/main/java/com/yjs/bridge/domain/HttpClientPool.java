...
public class HttpClientPool implements IHttpTransfer {
    ...
    public Pair<Integer, JSONObject> post(HttpPostParam httpParam) throws IOException, NoSuchAlgorithmException, KeyManagementException {
        //请求单例HttpClient
        ...
        try {
            return Pair.of(response.getStatusLine().getStatusCode(), JSON.parseObject(EntityUtils.toString(response.getEntity())));
        } finally {
    ...
    public Pair<Integer, JSONObject> get(HttpGetParam httpParam) throws IOException, NoSuchAlgorithmException, KeyManagementException {
        //请求单例HttpClient
        HttpClient client = getHttpsClient();
        //发送get请求
    ...
    public static RequestConfig getRequestConfig(HttpParam httpParam) {
